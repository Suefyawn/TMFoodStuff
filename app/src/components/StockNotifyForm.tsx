    ...
    return (
      <div className="w-full flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl px-5 py-4">
        ...
        <p className="text-green-700 font-semibold text-sm">
          {lang === 'ar'
            ? `سنرسل لك بريداً إلكترونياً عند توفّر ${productName} مرة أخرى!`
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-gray-500 font-medium">
        {lang === 'ar' ? 'أعلمني عند توفّر هذا المنتج مرة أخرى:' : 'Get notified when this is back in stock:'}
      </p>
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        >
          <Bell size={16} />
          {status === 'loading' ? '...' : lang === 'ar' ? 'أعلمني' : 'Notify Me'}
        </button>
      </form>
      {status === 'error' && (
