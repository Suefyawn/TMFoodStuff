  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-w-3xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={saveSettings} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setSettings({...settings, free_delivery: settings.free_delivery === 'true' ? 'false' : 'true'})} className={`w-10 h-6 rounded-full transition-colors flex items-center ${settings.free_delivery === 'true' ? 'bg-emerald-600 justify-end' : 'bg-gray-700 justify-start'}`}>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
            ...
            {promoCodes.map((promo, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 bg-gray-800 rounded-xl px-4 py-3">
                <div className="flex flex-wrap items-center gap-3 gap-y-1">
                  ...
                  <span className="text-emerald-400 text-sm font-bold">{promo.discount_percent}% off</span>
                  <span className={`text-xs ${promo.is_active ? 'text-emerald-500' : 'text-red-400'}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:items-end">
          <div className="flex-1">
            ...
            <input value={newPromo.code} onChange={e => setNewPromo({...newPromo, code: e.target.value.toUpperCase()})} placeholder="SUMMER25" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500 font-mono" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-[6rem_10rem] gap-2">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Discount %</label>
              <input type="number" value={newPromo.discount_percent} onChange={e => setNewPromo({...newPromo, discount_percent: Number(e.target.value)})} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500" />
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Expires</label>
              <input type="date" value={newPromo.expires_at} onChange={e => setNewPromo({...newPromo, expires_at: e.target.value})} style={{ colorScheme: 'dark' }} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500" />
            </div>
          </div>
          <button onClick={addPromo} disabled={!newPromo.code} className="px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded-xl hover:bg-emerald-500 disabled:opacity-50 sm:self-end">
            <Plus size={16} />
          </button>
        </div>
      </div>
...
function SettingInput({ label, value, onChange, type = 'text' }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500" />
    </div>
  )
}
