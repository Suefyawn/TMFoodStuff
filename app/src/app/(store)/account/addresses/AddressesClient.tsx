...
export default function AddressesClient({ initialAddresses }: { initialAddresses: Address[] }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 md:py-14" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {editingId !== null && (
        <div className="bg-white border border-stone-100 rounded-2xl p-6 shadow-sm mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-stone-600 mb-1.5">{isAr ? 'تسمية (اختياري)' : 'Label (optional)'}</label>
              <input value={form.label || ''} onChange={e => setForm(f => ({ ...f, label: e.target.value }))}
                placeholder={isAr ? 'المنزل، المكتب…' : 'Home, Office…'}
                className="w-full border-2 border-stone-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-forest-light" />
            </div>
            <div>
              <label className="block text-xs font-bold text-stone-600 mb-1.5">{isAr ? 'الإمارة' : 'Emirate'} *</label>
              <select value={form.emirate || ''} onChange={e => setForm(f => ({ ...f, emirate: e.target.value }))}
                className="w-full border-2 border-stone-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-forest-light" required>
                <option value="">{isAr ? 'اختر الإمارة' : 'Select emirate'}</option>
                {emirates.map(e => <option key={e} value={e}>{e}</option>)}
              </select>
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-stone-600 mb-1.5">{isAr ? 'المنطقة' : 'Area'} *</label>
              <input value={form.area || ''} onChange={e => setForm(f => ({ ...f, area: e.target.value }))}
                placeholder={isAr ? 'جميرا، المرسى…' : 'JLT, Marina, Khalidiyah…'}
                className="w-full border-2 border-stone-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-forest-light" required />
            </div>
            <div>
              <label className="block text-xs font-bold text-stone-600 mb-1.5">{isAr ? 'المبنى / الفيلا' : 'Building / Villa'}</label>
            ...
            <div>
              <label className="block text-xs font-bold text-stone-600 mb-1.5">{isAr ? 'مكاني (اختياري)' : 'Makani (optional)'}</label>
          ...
          <div className="flex items-center gap-2 mt-5">
            ...
            <button onClick={() => setEditingId(null)} className="text-stone-600 hover:text-stone-900 font-semibold px-3 py-2.5 text-sm">
              {isAr ? 'إلغاء' : 'Cancel'}
            </button>
          </div>
        </div>
      ...
      {addresses.length === 0 && editingId === null ? (
        <div className="bg-white border border-stone-100 rounded-2xl p-10 text-center shadow-sm">
          <div className="w-14 h-14 bg-stone-50 rounded-2xl flex items-center justify-center mx-auto mb-3">
            <MapPin size={22} className="text-stone-300" aria-hidden="true" />
          </div>
          <p className="text-stone-700 font-bold mb-1">{isAr ? 'لا توجد عناوين بعد' : 'No saved addresses yet'}</p>
          <p className="text-sm text-stone-500 mb-5">{isAr ? 'احفظ عنواناً لتوفير الوقت في كل عملية شراء.' : 'Save an address to speed up future checkouts.'}</p>
        </div>
      ) : (
        <ul className="space-y-3">
          {addresses.map(a => (
            <li key={a.id} className="bg-white border border-stone-100 rounded-2xl p-5 shadow-sm flex items-start justify-between gap-3 flex-wrap">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <h3 className="font-bold text-stone-900">{a.label || (isAr ? 'العنوان' : 'Address')}</h3>
                  {a.is_default && (
                ...
                {a.makani && <p className="text-xs text-stone-500 mt-0.5">Makani: {a.makani}</p>}
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => startEdit(a)} aria-label={isAr ? 'تعديل' : 'Edit'} className="p-2 text-stone-500 hover:text-forest-dark transition-colors">
                  <Pencil size={16} aria-hidden="true" />
                </button>
                <button onClick={() => remove(a.id)} aria-label={isAr ? 'حذف' : 'Delete'} className="p-2 text-stone-500 hover:text-red-600 transition-colors">
