...
export default async function UnsubscribePage({ searchParams }: Params) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-20 text-center">
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5 ${ok ? 'bg-green-100' : invalid ? 'bg-amber-100' : 'bg-stone-100'}`}>
        {ok
          ? <CheckCircle size={32} className="text-forest" aria-hidden="true" />
          : invalid
            ? <AlertCircle size={32} className="text-amber-600" aria-hidden="true" />
            : <Mail size={32} className="text-stone-400" aria-hidden="true" />}
      </div>
      ...
      <p className="text-stone-500 text-sm mb-6 max-w-sm mx-auto">
        {ok
          ? <>We won&apos;t send marketing emails to <strong className="text-stone-700">{email}</strong> anymore. You&apos;ll still receive transactional emails about orders you place.</>
          : invalid
      ...
      <Link
        href="/"
        className="inline-flex items-center gap-2 bg-stone-900 hover:bg-stone-700 text-white font-bold px-5 py-3 rounded-xl transition-colors"
      >
        Back to shop
      </Link>
    </div>
